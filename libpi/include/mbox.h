...
static inline uint32_t 
mbox_query(unsigned channel, volatile void *data) {
  mbox_write(MBOX_CH, data);
  uint32_t v = GET32(mbox_read(MBOX_CH));
...
serialno_trunc() {
  ...
  u[2] = 0x00010004; // serial tag
  u[3] = 8;          // response buffer size
  u[4] = 0;          // request code
  ...
  // uint64_t hi = u[6];
  // return (hi << 32) | lo;
  // NOTE: We simply discard the high bits for now
  return u[5];
}
