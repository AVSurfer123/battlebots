...
void list_init(struct list *list) {
    demand(list != NULL, assertion failed);
    list->head.prev = NULL;
...
struct list_elem *list_next(struct list_elem *elem) {
    demand(is_head(elem) || is_interior(elem), assertion failed);
    return elem->next;
}
...
struct list_elem *list_prev(struct list_elem *elem) {
    demand(is_interior(elem) || is_tail(elem), assertion failed);
    return elem->prev;
}
...
void list_insert(struct list_elem *before, struct list_elem *elem) {
    // debug_backtrace();
    demand(is_interior(before) || is_tail(before), assertion failed);
    demand(elem != NULL, assertion failed);
...
void list_splice(struct list_elem *before, struct list_elem *first,
    ...
    demand(is_interior(first), assertion failed);
    demand(is_interior(last), assertion failed);
...
struct list_elem *list_remove(struct list_elem *elem) {
    demand(is_interior(elem), assertion failed);
...
struct list_elem *list_pop_front(struct list *list) {
    // if (list == 0xc0105150) printf ("got into list 1\n");
    struct list_elem *front = list_front(list);
    // if (list == 0xc0105150) printf ("got into list 2, elem is %p\n", front);
    list_remove(front);
    // if (list == 0xc0105150) printf ("got into list 3, removed elem %p\n",
    // front);
    return front;
}
...
struct list_elem *list_front(struct list *list) {
    demand(!list_empty(list), assertion failed);
    return list->head.next;
}
...
static struct list_elem *find_end_of_run(struct list_elem *a,
                                         struct list_elem *b,
                                         list_less_func *less, void *aux) {
    demand(a != NULL, assertion failed);
    demand(b != NULL, assertion failed);
    demand(less != NULL, assertion failed);
    demand(a != b, assertion failed);
...
static void inplace_merge(struct list_elem *a0, struct list_elem *a1b0,
                          ...
                          void *aux) {
    demand(a0 != NULL, assertion failed);
    demand(a1b0 != NULL, assertion failed);
    demand(b1 != NULL, assertion failed);
    demand(less != NULL, assertion failed);
    demand(is_sorted(a0, a1b0, less, aux), assertion failed);
    demand(is_sorted(a1b0, b1, less, aux), assertion failed);
...
void list_sort(struct list *list, list_less_func *less, void *aux) {
    ...
    demand(is_sorted(list_begin(list), list_end(list), less, aux), assertion failed);
}
...
struct list_elem *list_find(struct list *list,
                            list_elem_predicate *list_elem_func, void *aux) {
    struct list_elem *e;
    for (e = list_begin(list); e != list_end(list); e = list_next(e)) {
        if (list_elem_func(e, aux)) return e;
    }
    return NULL;
}
